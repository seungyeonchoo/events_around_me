...
const StatusItem = ({ id, date, status }: StatusItemProps) => {
  ...
  const common_styles =
    'relative absolute cursor-pointer w-[6rem] h-[6rem] rounded-full font-bold flex items-center justify-center border-2 shadow-lg max-sm:w-[3.5rem] max-sm:h-[3.5rem] text-[1.5rem] max-sm:text-[1rem] p-1';
  ...
  return (
    <label className={`${common_styles} ${status_styles}`}>
      ...
      <span className="absolute peer-hover:invisible">{id === 0 ? 'START' : `DAY ${id}`}</span>
      <span className="peer-hover:visible invisible text-center text-[1rem] max-sm:text-[0.3rem]">
        {date}
      </span>
    </label>
  );
